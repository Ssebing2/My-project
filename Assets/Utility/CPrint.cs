...
- 프린트는 콘솔 프로젝트에서 사용한 것처럼 출력 규칙을 유니티 스럽게 바꾼 버전
...
유니티에서는... globla using을 권장하지 않는다.
 ㄴ 이놈 자체가 자동화가 완벽하게 안된다.


● 유니티에서 전역 스럽게 (느낌) 사용하고 싶다.
...
public static class CPrint
{
    ...
    private const int INDENT_SPACESS = 2;
    ...
    - 해시 코드를 기준으로 저장 위치(버킷)를 고른다.
     ㄴ "해시코드 % 버킷개수" 같은 방식으로 인덱스를 결정한다고 생각하면 된다.
    ...
    4. 재해싱
    - 안에 요소가 많아지면 버킷이 뻑뻑해 진다. → 성능이 떨어질 수 있음
    ...
    */
    ...
    private static string Indent
    {
        ...
        {
            // 레벨 * 공백수
            return new string(' ', _indentLevel * INDENT_SPACESS);
        }
    }
    ...
    private static void Emit(ELogKind kind, string msg, string tag = null, string colorHex = null)
    {
        ...
        // 색상값 → 헥스를 쓰는 이유 → 가장 범용적인 방식 (문자열로 색을 표현하기에 가장 무난)
        //  ㄴ 1. 표준이고 무난함
        //  ㄴ 2. 문자열 형식이기 때문에 로그 포맷을 만들 때 바로 끼워 넣기가 좋음
        //  ㄴ 3. 16진수라서 압축이 잘됨 (RGB)
        ...
        // 단, tagr가 null or 빈 문자열이면 접두사 없이 msg만 출력
        string prefix = string.Empty;

        if (!string.IsNullOrEmpty(tag))
        {
            // t이고 colorHex가 주어졌으면 tag 부분만 색을 입히곘다. → 가독성
    ...
    {
        Emit(ELogKind.Warn, msg, "WARN", "##FF9100");   // 주황 계열
    }
    public static void Error(string msg)
    {
        Emit(ELogKind.Error, msg, "Error", "#FF1744");  // 빨강 계열
    }

    public static void Success(string msg)
    {
        Emit(ELogKind.Success, msg, "OK", "#00C853");   // 초록 계열
    }

    // Assert
    public static void Assert(bool condition, string msg)
    {
        // if문이 많이 빠질 수 있음 → 매번 if문 하는것 보다 Assert로 빼는것이 좋음
    ...
    public static T Ref<T>(T obj, string msg) where T : class
    {
        ...
        T Ref<T> (T obj, string msg)
         ㄴ obj가 null이면 경고를 하고 obj 그대로 반환
         ㄴ 검사 + 반환을 한번에 하고 싶다.

        왜 T인가
         ㄴ 한줄로 끝내기 위해서, 이후 어떤 타입이 들어올지 예측은 안되지만 클래스 타입임을 명시하기 위해서
         ㄴ 동작시키기 위해서
        ...
        _rb = CPrint.Ref(GetComponnet<Rigidbody>, "...");

        - 필수 컴포넌트를 체크하고 바로 대입을 해주기 위함

        ● 제네릭

        - 제네릭은 타입을 나중에 정하는 설계
         ㄴ 호출할 때 타입이 결정된다.
         ㄴ(컴파일 시점에 데이터 타입을 확인하여 오류를 방지한다.)
        ...
        - 제네릭은 <T>와 같은 제네릭 타입을 명시함으로서 정의 가능
        ...
        - 제네릭 프로그래밍으로 전환이 되면 설계가 더욱 까다로워진다.

        ● where T : class

        C#기준

        - 기본적으로 모든 데이터 타입에 동작하도록 설계가 되어야 한다.
        ...
        - T는 클래스만 받겠다는 제한 (참조형)
         ㄴ 지금 만든 함수는 null 체크가 핵심인데 int / float 같은것이 들어오면 피곤해지기때문

        - where T : class로 null이 될 수 있는 타입만 허용 하겠다. → 실수 선행 방지용

        ※ 단순한 클래스 설계가 아닌 lib + Framework 설계에서 사용된다.
        ...
        class CSomeClass<T> where T : SomeInterface
         ㄴ 타입을 SomeInterface를 직,간접적으로 따르는 형식으로 제한
    ...
    public static void KV(string key, object value)
    {
        ...
        // 우리가 디버깅할때 → 가장 자주 찍는 형태 → 여기서 포맷 통일하겠다.
    ...
    public static void Group(string title, Action body, char lineCh = '=', int LineCount = 20)
    {
        ...
        // 타이틀 / 들여쓰기 올리기 / 본문 실행 / 들여쓰기 내리기 / 구분선으로 마무리

        /*
        ▶ 델리게이트 → 간단 버전

        - 함수를 변수처럼 다룰수 있게 해주는 타입
        ...
        - Action은 코드를 데이터처럼 넘긴다 라는 느낌 → Group함수는 로그 꾸러미를 함수로 받는것
